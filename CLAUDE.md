...
- Blocklist: `rejected_words(word, action)` hard-blocks resubmission with 409 for regular users. Moderators instead get `409 {requiresConfirmation: true, prior}` (also when reverting a recent deliberate opposite decision — `lib/prior-decisions.ts`); the client re-sends with `force: true` and a mandatory comment that is stored as a `word_comments` row. Client helper: `app/components/suggestion-api.ts`.
